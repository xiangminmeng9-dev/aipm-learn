...
export default function AiPmArticlesPage() {
  ...
  const fetchArticles = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiFetch('/api/rss/articles?category=ai_pm');
      if (res.ok) {
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    try {
      await apiFetch('/api/rss/refresh?category=ai_pm', { method: 'POST' });
      await fetchArticles();
    } catch { /* ignore */ }
    setRefreshing(false);
  };
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        {article.impact && expanded && (
          <div className="mt-3 rounded-lg bg-amber-50/60 p-2.5">
            <p className="text-xs font-medium text-amber-700">对产品经理意味着什么</p>

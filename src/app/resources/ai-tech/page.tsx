    ...
    <div className="flex h-full flex-col bg-[#F8F9FB]">
      ...
      <div className="shrink-0 border-b border-[#E5E7EB] bg-white px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-semibold text-[#1F2937]">AI 技术动态</h1>
            <p className="text-xs text-[#6B7280]">追踪 RAG / Agent / LLM 技术路线与前沿实现</p>
          </div>
          <button
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {loading ? (
          <div className="flex items-center justify-center py-20 text-sm text-[#9CA3AF]">加载中...</div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-sm text-[#9CA3AF]">
            <p>暂无文章</p>
            <button onClick={handleRefresh} className="mt-2 text-xs text-[#4F46E5] hover:underline">点击采集</button>
          </div>
        ) : (
    ...
    <div className="group flex flex-col rounded-xl border border-[#E5E7EB] bg-white transition hover:border-indigo-200 hover:shadow-sm">
      <div className="flex flex-1 flex-col p-4">
        <div className="flex items-start justify-between gap-2">
          <h3 className="flex-1 text-sm font-medium leading-snug text-[#1F2937] group-hover:text-indigo-600 transition line-clamp-2">
            {article.link ? (
              <a href={article.link} target="_blank" rel="noopener noreferrer">{article.title}</a>
            ) : article.title}
          </h3>
          {article.link && (
            <a href={article.link} target="_blank" rel="noopener noreferrer" className="shrink-0 rounded p-1 text-[#9CA3AF] hover:text-indigo-600 transition">
        ...
        {article.plainExplanation && (
          <p className="mt-2 text-xs leading-relaxed text-[#6B7280] line-clamp-3">{article.plainExplanation}</p>
        )}
        {article.impact && expanded && (
          <div className="mt-3 rounded-lg bg-amber-50/60 p-2.5">
            ...
            <p className="mt-0.5 text-xs text-[#374151]">{article.impact}</p>
          </div>
        )}

...
export async function POST(request: NextRequest) {
  try {
    ...
    const { keyword, target_count = 100, date_from, date_to } = body;
    ...
    const cappedCount = Math.min(Math.max(target_count, 10), 500); // Cap between 10-500 per request

    const result = await crawlBossJds({

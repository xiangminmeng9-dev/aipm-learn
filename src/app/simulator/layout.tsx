...
const NAV_ITEMS = [
  { href: '/simulator', label: '工作流程', icon: '🚀' },
  { href: '/simulator/project', label: '项目实战沙盒', icon: '🏗️' },
  { href: '/simulator/boss-1v1', label: 'Boss 1V1', icon: '👔' },
];

export default function SimulatorLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  // Match /simulator/project/xxx to /simulator/project
      ...
      <aside className="sticky top-0 hidden h-screen w-56 shrink-0 border-r border-border bg-card lg:block">
        <div className="flex h-full flex-col">
          <div className="border-b border-border px-5 py-5">
            ...
            <h1 className="mt-2 text-lg font-bold text-foreground">模拟器</h1>
            <p className="text-xs text-muted-foreground">沉浸式 AI PM 实战</p>
          </div>
              ...
              return (
                <Link
                  ...
                  className={`rounded-lg px-3 py-1.5 text-xs transition-colors ${
                    isActive ? 'bg-teal-50 font-semibold text-teal-700' : 'text-muted-foreground hover:bg-muted'
                  }`}
                >

...
function markdownToPlainText(md: string): string {
  return md
    ...
    // e.g. *text* always matches, _text_ only when preceded by whitespace/start-of-line
    .replace(/\*(.+?)\*/g, '$1')
    ...
    .replace(/^[\s]*[-*+]\s+/gm, '• ')
    // Convert ordered list markers (1. 2. etc.) to keep as-is
    // Remove HTML tags
    .replace(/<[^>]+>/g, '')
...
function parseChangesSummary(summary: string): ChangeItem[] {
  ...
  try {
    ...
    // Not JSON, parse the string format (legacy)
  }

  // Parse 【维度】xxx format (from old normalizeSummary)
...
export default function ResumeResult({ modifiedResume, changesSummary }: ResumeResultProps) {
  ...
  const items = parseChangesSummary(changesSummary);
  ...
  return (
    <div className="space-y-4">
      ...
      {changesSummary && (
        <div className="overflow-hidden rounded-xl border border-indigo-200 bg-indigo-50/30 dark:border-indigo-700 dark:bg-indigo-950/30">
          <button
            onClick={() => setIsSummaryExpanded(!isSummaryExpanded)}
            className="flex w-full items-center justify-between px-5 py-3.5 text-left hover:bg-indigo-50/50 dark:hover:bg-indigo-950/50 transition-colors"
          >
            <span className="flex items-center gap-2 text-sm font-semibold text-indigo-700 dark:text-indigo-300">
              ...
              修改摘要
              {items.length > 0 && <span className="text-xs font-normal text-indigo-500 dark:text-indigo-400">{items.length}项改动</span>}
            </span>
          ...
          <AnimatePresence>
            {isSummaryExpanded && (
              ...
              >
                <div className="border-t border-indigo-200 dark:border-indigo-700 px-5 py-4 space-y-3">
                  {sortedDimensions.map(dim => {
                    ...
                    return (
                      <div key={dim} className="space-y-1.5">
                        ...
                        {/* Items under this dimension */}
                        <div className="ml-2 space-y-2">
                          {groupItems.map((item, i) => (
                            <div key={i} className="text-xs leading-relaxed">
                              {/* Location tag */}
                              {item.location && (
                                <div className="text-muted-foreground mb-0.5">📍 {item.location}</div>
                              )}
                              {/* Diff: before → after */}

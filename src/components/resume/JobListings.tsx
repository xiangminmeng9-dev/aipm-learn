  ...
  if (jobs.length === 0) {
    return (
      <div className="rounded-xl border border-[#E5E7EB] bg-white p-8 text-center">
        ...
        <p className="text-sm text-[#6B7280]">暂无推荐职位</p>
        <p className="mt-1 text-xs text-[#9CA3AF]">稍后再来看看</p>
      </div>
    );
  }
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      {jobs.map((job, index) => (
        <motion.div
          ...
          className="group rounded-xl border border-[#E5E7EB] bg-white p-5 transition-all duration-200 hover:shadow-md hover:border-indigo-200"
        >
          <div className="mb-3 flex items-start justify-between">
            <div>
              <h3 className="text-sm font-semibold text-[#1F2937] line-clamp-1">{job.title}</h3>
              <p className="mt-0.5 text-sm text-[#6B7280]">{job.company}</p>
            </div>
          ...
          <div className="mb-3 flex items-center gap-1.5 text-xs text-[#9CA3AF]">

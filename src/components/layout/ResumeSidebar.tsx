...
const navItems = [
  { label: '数据看板', href: '/resume/dashboard', icon: (
...
export default function ResumeSidebar() {
  ...
  return (
    <aside className="flex h-full flex-col border-r bg-sidebar" style={{ backgroundColor: 'var(--sidebar-resume)', borderColor: 'var(--sidebar-resume-border)' }}>
      <div className="border-b px-5 py-5" style={{ borderColor: 'var(--sidebar-resume-border)' }}>
        ...
        <h2 className="mt-3 text-lg font-semibold text-foreground">简历助手</h2>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              style={isActive ? { backgroundColor: 'var(--sidebar-resume-accent)', color: 'var(--sidebar-resume-active)' } : {}}
            >
              <span style={isActive ? { color: 'var(--sidebar-resume-active)' } : {}}>{item.icon}</span>
              <span>{item.label}</span>
              {isActive && <span className="ml-auto h-1.5 w-1.5 rounded-full" style={{ backgroundColor: 'var(--sidebar-resume-active)' }} />}
            </Link>
          );
        })}
      </nav>

      <div className="border-t px-5 py-4" style={{ borderColor: 'var(--sidebar-resume-border)' }}>

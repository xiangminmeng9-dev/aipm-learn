  ...
  if (articles.length === 0) {
    return (
      <div className="rounded-2xl border border-[#E5E7EB] bg-white py-12 text-center">
        <p className="text-lg text-[#6B7280]">暂无新闻数据</p>
        <p className="mt-1 text-base text-[#9CA3AF]">数据将在首次访问时自动抓取</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {articles.map((article, i) => (
        <div
          key={article.id}
          className="group rounded-xl border border-[#E5E7EB] bg-white px-4 py-3 transition hover:border-indigo-100 hover:shadow-sm"
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {article.summary && (
                <p className="mt-1 text-base text-[#6B7280]">{article.summary}</p>
              )}
            ...
            {article.url && (
              <a
                ...
                className="shrink-0 rounded-lg p-2 text-[#9CA3AF] opacity-0 transition group-hover:opacity-100 hover:bg-indigo-50 hover:text-indigo-600"
              >
